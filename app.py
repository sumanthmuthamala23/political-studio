...
# Download Telugu Unicode Font (Suranna) for error-free Telugu text rendering
...
# Helper function to create circular-cropped portrait images
...
# Leader Reference Photo Fallbacks (Wikimedia Public Domain / Official BRS)
LEADER_URLS = {
...
def load_or_fetch_leader(uploaded_file, default_url):
    if uploaded_file is not None:
        return Image.open(uploaded_file)
    try:
        res = requests.get(default_url, timeout=5)
        return Image.open(BytesIO(res.content))
    except Exception:
        # Fallback dummy placeholder
        return Image.new("RGBA", (200, 200), BRS_PINK)

# Sidebar UI (In English)
    ...
    with col2:
        ...
        if st.button("Generate Telugu Poster", type="primary"):
            with st.spinner("Rendering poster with high-resolution typography..."):
                ...
                # --- TOP LEADERS ROUNDED BADGES ---
                # Top Left: KCR Garu
                img_kcr = load_or_fetch_leader(kcr_file, LEADER_URLS["kcr"])
                ...
                # Top Right: KTR Garu & Harish Rao Garu
                img_ktr = load_or_fetch_leader(ktr_file, LEADER_URLS["ktr"])
                ...
                img_harish = load_or_fetch_leader(harish_file, LEADER_URLS["harish"])
                ...
                # --- CENTER / MAIN SUBJECT: TATA MADHUSUDHAN GARU ---
                if tata_file is not None:
                    ...
                    tw, th = tata_img.size
                    # Placed prominently below top leadership, grounded at bottom ribbon
                    poster.paste(tata_img, (W - tw - 30, H - th - 210), tata_img)

                # --- TELUGU TYPOGRAPHY SECTION ---
                ...
                # --- BOTTOM DESIGNATION & GREETING STRIP ---
                ...
                with open(output_file, "rb") as f:
                    st.download_button("Download Telugu HD Poster", f, "tata_madhusudhan_poster.png", "image/png")
        ...
        if not video_uploads and not photo_uploads:
            ...
            with st.spinner("Processing media tracks and stitching reel..."):
                ...
                with open(output_reel_name, "rb") as f:
                    st.download_button("Download Mixed Video Reel", f, "tata_madhusudhan_mixed_reel.mp4", "video/mp4")import streamlit as st
from PIL import Image, ImageDraw, ImageFont, ImageOps
import os
import requests
from moviepy.editor import VideoFileClip, ImageClip, AudioFileClip, concatenate_videoclips
